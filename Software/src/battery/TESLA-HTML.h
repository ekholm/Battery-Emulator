...
class TeslaHtmlRenderer : public BatteryHtmlRenderer {
 public:
  /* Which extended struct to render, handed over by the battery that owns this
     renderer. A second Tesla pack has none, and nullptr here is what makes its
     page say so instead of quietly showing the first pack's numbers.

     renders_own_battery_data() stays false, so the advanced page shows its
     "limited to the Main Battery" notice for pack 2 and never calls the code
     below - a better message than anything this renderer can produce. The
     nullptr branch is what makes that a choice rather than the only safe
     option: any future caller reaching a second instance directly gets an
     honest empty section instead of pack 1's numbers. */
  void set_extended(DATALAYER_INFO_TESLA* extended) { tesla = extended; }

  String get_status_html() {
    String content;

    if (!tesla) {
      content += "<h4>No extended data for this battery</h4>";
      return content;
    }
    ...
    content +=
        "<h4>Contactors Request Status: " + String(HVP_contactor[tesla->battery_packCtrsRequestStatus]) + "</h4>";
    ...
    content += "<h4>Initial Precharge Substate: " + String(PCS_dcdcSubState[tesla->PCS_dcdcInitialPrechargeSubState]) +
               "</h4>";
    //0x3C4 PCS_info
    ...
    appendFault(content, "HVP_gpioPackCoontPosFlywheel", tesla->HVP_gpioPackCoontPosFlywheel);
    appendFault(content, "HVP_gpioCpLatchEnable", tesla->HVP_gpioCpLatchEnable);
    appendFault(content, "HVP_gpioPcsEnable", tesla->HVP_gpioPcsEnable);
    appendFault(content, "HVP_gpioPcsDcdcPwmEnable", tesla->HVP_gpioPcsDcdcPwmEnable);
    appendFault(content, "HVP_gpioPcsChargePwmEnable", tesla->HVP_gpioPcsChargePwmEnable);
    ...
    appendFault(content, "HVP_gpioSecDrdy", tesla->HVP_gpioSecDrdy);
 ...
 private:
  DATALAYER_INFO_TESLA* tesla = nullptr;
};
