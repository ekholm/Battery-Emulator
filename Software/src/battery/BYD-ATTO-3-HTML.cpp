...
String BydAtto3HtmlRenderer::get_dtc_html() {
...
String BydAtto3HtmlRenderer::get_status_html() {
  ...
  if (s.length() > 0) {
    ...
    if (byd_datalayer->balancing_state != 0) {
      ...
      content += value_td;
      content += (byd_datalayer->balancing_state == 5) ? "<span style='color:#d29922'>" : "<span style='color:white'>";
      content += hold_text;
    ...
    if (byd_datalayer->charge_session_state == 0) {
      ...
      content += "<span style='font-weight:normal;color:#8b949e'> (" + String(byd_datalayer->charge_grant) + ")</span>";
    } else {
...
void BydAtto3HtmlRenderer::append_balance_time_html(CheckedHtml& out) const {
  out +=
