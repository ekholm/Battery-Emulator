...
void TeslaBattery::
  ...
  /* The part-number query is armed off the serial number, but it drives a UDS
     request on the wire rather than anything on the web page, so it runs for
     every instance - including one with no extended struct to publish into. */
  if (parsed_battery_serialNumber && battery_serialNumber[13] != 0 && !parsed_battery_partNumber &&
  ...
  /* Everything below publishes into the extended struct for the advanced page.
     A second Tesla pack has none - it shares the datalayer with the first - so
     the whole block is skipped rather than letting two packs interleave into
     one struct. Same shape as ECMP-BATTERY.cpp. */
  if (datalayer_tesla) {
...
void TeslaBattery::printFaultCodesPcsCp() {
  ...
  if (!datalayer_tesla) {
    // A battery with no extended struct never filled these arrays in.
    return;
  }
  for (int i = 0; i < 94; i++) {
