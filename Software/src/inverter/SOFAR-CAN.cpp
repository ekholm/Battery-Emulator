...
void SofarInverter::
  ...
  SOFAR_351.data.u8[0] = (datalayer.batteries[0].info.max_design_voltage_dV & 0x00FF);
  SOFAR_351.data.u8[1] = (datalayer.batteries[0].info.max_design_voltage_dV >> 8);
  SOFAR_351.data.u8[2] = (datalayer.batteries[0].status.max_charge_current_dA & 0x00FF);
  SOFAR_351.data.u8[3] = (datalayer.batteries[0].status.max_charge_current_dA >> 8);
  SOFAR_351.data.u8[4] = (datalayer.batteries[0].status.max_discharge_current_dA & 0x00FF);
  SOFAR_351.data.u8[5] = (datalayer.batteries[0].status.max_discharge_current_dA >> 8);
  // Minvoltage (eg 300.0V = 3000 , 16bits long) Discharge Cutoff Voltage
  SOFAR_351.data.u8[6] = (datalayer.batteries[0].info.min_design_voltage_dV & 0x00FF);
  SOFAR_351.data.u8[7] = (datalayer.batteries[0].info.min_design_voltage_dV >> 8);

  // ----- Frame 0x355 – SoC / SoH -----
  // SoC deception only to CAN (we do not touch datalayer)
  uint16_t spoofed_soc = datalayer.batteries[0].status.reported_soc;  // 0..10000 pptt
  if (spoofed_soc >= 10000) {
    spoofed_soc = 9900;  // limit to 99%
  }
  SOFAR_355.data.u8[0] = spoofed_soc / 100;                             // %
  SOFAR_355.data.u8[2] = datalayer.batteries[0].status.soh_pptt / 100;  // %

  // ----- Frame 0x356 – pack voltage/current/temp -----
  // Voltage (e.g. 370.0V -> 3700 dV), Current in dA, Temperature in dC
  SOFAR_356.data.u8[0] = (datalayer.batteries[0].status.voltage_dV & 0x00FF);
  SOFAR_356.data.u8[1] = (datalayer.batteries[0].status.voltage_dV >> 8);
  SOFAR_356.data.u8[2] = (datalayer.batteries[0].status.reported_current_dA & 0x00FF);
  SOFAR_356.data.u8[3] = (datalayer.batteries[0].status.reported_current_dA >> 8);
  SOFAR_356.data.u8[4] = (datalayer.batteries[0].status.temperature_max_dC & 0x00FF);
  SOFAR_356.data.u8[5] = (datalayer.batteries[0].status.temperature_max_dC >> 8);
  ...
  if (datalayer.batteries[0].info.max_design_voltage_dV > 20) {  //div0 protection
    calculated_capacity_AH = (datalayer.batteries[0].info.reported_total_capacity_Wh /
                              (datalayer.batteries[0].info.max_design_voltage_dV * 0.1));
  }
...
void SofarInverter::map_can_frame_to_variable(CAN_frame rx_frame) {
  switch (rx_frame.ID) {
    ...
    case 0x705: {
      ...
      if (target_m != datalayer.batteries[0].settings.sofar_user_specified_battery_id) {
      ...
      auto stamp_ids = [&](CAN_frame& f) {
        ...
        f.data.u8[0] = datalayer.batteries[0].settings.sofar_user_specified_battery_id;  // PACK ID
...
bool SofarInverter::setup() {  // Performs one time setup at startup over CAN bus
  ...
  uint16_t base_offset = (datalayer.batteries[0].settings.sofar_user_specified_battery_id << 12);
