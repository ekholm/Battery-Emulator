...
#define SETTINGS_TABLE_ROWS(S_INT, S_STR)                                                                              \
  ...
  S_INT(CTINVERT,       "CTINVERT",       "CTINVERT",       BoolU8, SF_REBOOT_REQUIRED,             0, 0, 1)   \
                                                                                                                       \
  /* --- Added upstream after this lane branched; rows derived from upstream's own code --- */                         \
  /* CHGSTARQ is the CHG_STA_RQ bit pattern. Its handler clamps >2 back to 0, and it is the one  */                    \
  ...
  /* The BYD Atto 3 rows are written from the battery page, not the settings page.                */                   \
  S_INT(BYDNATTERM,     "BYDNATTERM",     nullptr,          BoolU8, SF_NONE,                        1, 0, 1)           \
  S_INT(BYDBALEN,       "BYDBALEN",       nullptr,          BoolU8, SF_NONE,                        0, 0, 1)           \
  S_INT(BYDBALMIN,      "BYDBALMIN",      nullptr,          U32,    SF_NONE,                       30, 1, 1440)
// clang-format on
