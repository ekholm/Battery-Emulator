...
That is enough to answer three questions mechanically:
  ...
  1. Which boards can even be confused? Chip family is a compile-time choice
     ...
     family. Flash size and PSRAM are free to read and risk nothing.
...
# GPIO numbers that only one chip family has. Not a heuristic: the ESP32-S3
# has no GPIO 22-25 at all, and the classic ESP32 stops at 39, so a declaration
# using one of these could not be describing the other part. Used only to place
# a board that no env builds - enough to keep it out of the groups it provably
# cannot be in, without pretending to know its flash size.
CLASSIC_ONLY = (22, 23, 24, 25)
S3_ONLY = tuple(range(40, 49))


def family_from_pins(roles):
    classic = sorted(g for g in roles if g in CLASSIC_ONLY)
    s3 = sorted(g for g in roles if g in S3_ONLY)
    if classic and not s3:
        return 'ESP32', f'declares GPIO {", ".join(map(str, classic))}, which the ESP32-S3 does not have'
    if s3 and not classic:
        return 'ESP32-S3', f'declares GPIO {", ".join(map(str, s3))}, which the classic ESP32 does not have'
    return None, None
...
def split_free(candidates, facts, macros, derived):
    """Partition by the discriminators that drive no pins at all.

    A board no env builds has no flash size to go on, so it cannot be placed in
    one group - but its pin numbers can still rule out a whole chip family, and
    that is worth using: it keeps such a board out of the groups it provably
    cannot be in instead of contaminating every one of them."""
    groups, floating = {}, []
    for board in candidates:
        f = facts.get(macros.get(board))
        if f is None:
            floating.append(board)
            continue
        groups.setdefault((f['family'], f['flash_mb'], f['psram']), []).append(board)
    for board in floating:
        family = derived.get(board, (None, None))[0]
        for key, members in groups.items():
            if family is None or key[0] == family:
                members.append(board)
...
def render(boards, facts, macros, roles, unplaced, probes, derived):
    L = []
    add = L.append
    ...
    add('|---|---|---|---|---|---|')
    for b in boards:
        f = facts.get(macros.get(b))
        if f is None:
            add(f'| {b} | `{macros.get(b, "?")}` | **none** | unknown | unknown | unknown |')
        else:
            add(f'| {b} | `{macros[b]}` | `{f["env"]}` | {f["family"]} | {f["flash_mb"]} MB | '
                f'{"yes" if f["psram"] else "no"} |')
    add('')
    orphans = [b for b in boards if macros.get(b) not in facts]
    for b in orphans:
        fam, why = derived.get(b, (None, None))
        add(f'> **`{b}` has a declaration and a header (`hal.cpp` switches on `{macros.get(b, "?")}`) '
            'but no `platformio.ini` env, so nothing builds it today.** Its flash size is therefore')
        if fam:
            add(f'> unknown, but its family is not a guess: it {why}, so it is an **{fam}** board. It')
            add(f'> is placed in every {fam} group and none of the others.')
        else:
            add('> unknown and its pin numbers do not settle the family either, so it is placed in '
                'every group - the conservative reading.')
        add('> Either way this is a gap worth closing: a board the tree can select but cannot build '
    ...
    add('')
    groups, floating = split_free(boards, facts, macros, derived)
...
def main():
    ...
    facts = platformio_facts()
    derived = {b: family_from_pins(roles[b]) for b in boards if macros.get(b) not in facts}
    report = render(sorted(boards), facts, macros, roles, unplaced, probes, derived)
