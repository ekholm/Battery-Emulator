 ...
 *
 * These read comm_can.cpp as source. Not because it is absent - it is compiled into this
 * binary, with emulated chips under the vendored drivers - but because the property is "every
 * writer of this flag maintains it", and a behavioural case only reaches the writers a board
 * arrangement happens to run. Behaviour that CAN be driven is driven, in comm_can_tests.cpp.
 */
namespace {
...
// The extraction and the stripper are utils/source_scan.h's - see the
// three properties documented there, one of which this file's own copy lacked.

}  // namespace

TEST(CanPauseStateSource, StoppingCanTakesTheFlagDownWithThePeripheral) {
  const std::string body = strip_comments(required_function_body(comm_can_source(), "void stop_can()"));
...
TEST(CanPauseStateSource, RestartingCanSetsTheFlagFromTheResultRatherThanDiscardingIt) {
  const std::string body = strip_comments(required_function_body(comm_can_source(), "void restart_can()"));
...
TEST(CanPauseStateSource, EveryNativeStartOrStopMaintainsTheFlag) {
  ...
  for (const std::string& fn : {std::string("void init_CAN()"), std::string("void stop_can()"),
    ...
    const std::string body = strip_comments(required_function_body(src, fn));
    EXPECT_NE(body.find("native_can_initialized ="), std::string::npos)
        << fn
...
TEST(CanPauseStateSource, SendingIsForbiddenBeforeThePeripheralIsStopped) {
  const std::string body = strip_comments(
      required_function_body(source_of("Software/src/devboard/safety/safety.cpp"), "void update_pause_state()"));
